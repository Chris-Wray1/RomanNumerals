 ...
 */
class RomanNumerals extends Controller
{
	...
	*/
	...
	protected static $VIEWS_BY_PLACE_VALUE = [
			['&#x2160;', '&#x2164;', '&#x2169;',], //ones
			['&#x2169;', '&#x216C;', '&#x216D;',], //tens
			['&#x216D;', '&#x216E;', '&#x216F;',], // hundreds
			['&#x216F;', '&#x2181;', '&#x2182;',], //thousands
			['&#x2182;', '&#x2187;', '&#x2188;',], //tens of thousands
			['&#x2188;', '~', '~',], //hundreds of thousands
			['~', '~', '~',], // millions. there are no values for the last two that I could find
		];

	/**
	* @var string[]  A map of numbers and their representative Roman numerals in notation format. This map allows us to make any numeral by replacing the the notation with the place value equivalent.	
	* @static
	*/
	...
	*
	* @static
	* @param int|string $number a number or numeric string less than 399,999
	* @throws \InvalidArgumentException if the provided $number argument is not numeric or greater than 399,999.
	* @return string Roman numeral equivalent	
	*/
	public static function number_to_numerals() {
		$number = (!empty($_POST['toconvert'])) ? $_POST['toconvert'] : 0;

		if(!is_numeric($number)) {
			return json_encode(['message' => "Invalid Number", 'method' => "INT to ROMAN", 'status' => "false"]);
		}
		if($number > 399999) {
			return json_encode(['message' => "Number cannot be greater than 399,999", 'method' => "INT to ROMAN", 'status' => "false"]);
		}
		...
		for($i = 0; $i < $length; $i++) {
			...
			$view_map = self::$VIEWS_BY_PLACE_VALUE[$i];
			$view = str_replace(self::$NOTATION, $view_map, self::$NUMBER_TO_NOTATION[$char]) . $view;
		}

		return json_encode(['result' => $numerals, 'view' => $view, 'method' => "INT to ROMAN", 'status' => "true"]);
	}
	...
	*/
	public static function numerals_to_number() {
		$number = 0;
		$numeral_string = (!empty($_POST['toconvert'])) ? strrev((string) $_POST['toconvert']) : '';
		...
		for($i = 0; $i < $length; $i++) {
			...
			if(!isset(self::$NUMERALS_TO_NUMBER[$char])) {
				return json_encode(['message' => "Invalid Roman Numeral", 'method' => "ROMAN to INT", 'status' => "false"]);
			}
		...
		return json_encode(['result' => $number, 'view' => $number, 'method' => "ROMAN to INT", 'status' => "true"]);
	}
